...
function VideoSlide({ item }: { item: Trailer }) {
    return (
        <div className="w-full h-full bg-black grid place-items-center" style={{ aspectRatio: '16/9' }}>
            <video 
...
const GameSwiperGallery = ({ gameId }: Props) => {
    const { data: trailers, error: trailersError, isLoading: trailersLoading } = useTrailers(gameId);
    const { data: screenshots, error: screenshotsError, isLoading: screenshotsLoading } = useScreenshots(gameId);
    ...
    const screenshotsItems: GalleryItem[] = (screenshots?.results ?? []).map((s) => 
        ({
            original: s.image,
            thumbnail: s.image,
